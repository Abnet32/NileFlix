...
import { type TMDBSeason, type TMDBSeriesDetails } from "@/lib/tmdb";
...
type SeasonDetailProps = {
  ...
  /** Where the "Back to show" button goes. */
  backHref: string;
  /** Builds a link for each episode (and the "open first episode" button). */
  episodeHref: (episodeNumber: number) => string;
};
      ...
      <section className="grid gap-8 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:items-center">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-wrap items-center gap-4 pt-1">
            {firstEpisodeNumber ? (
              <Link
                href={episodeHref(firstEpisodeNumber)}
                className={cn(
        ...
        <div className="grid gap-3 sm:grid-cols-4">
          {episodes.map((episode, index) => {
            ...
            return (
              <Link
                key={episode.id}
                href={episodeHref(episode.episode_number)}
                className={cn(
