    ...
    return (
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="animate-pulse rounded-sm border border-border/60 bg-card/60 p-4">
  ...
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
      {statItems.map(({ label, value, icon: Icon }) => (
        <div
          key={label}
          className="flex flex-col items-center gap-2 rounded-sm border border-border/60 bg-card/60 p-5 text-center shadow-sm backdrop-blur transition-all duration-200 hover:border-border hover:shadow-md"
        >
